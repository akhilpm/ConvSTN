...
class Net(nn.Module):
    ...
    def forward(self, x, epoch, flag, log, batch_id=0, batch_size=4):
        ...
        with torch.no_grad():
            all_boxes = box_utils.get_transformed_bounding_box_cub(self.box_pos, theta, data, self.grid_positions_is[0], H, W, epoch, fmap, x)
    
        #all_boxes = []
        return classif_scores, x, all_boxes