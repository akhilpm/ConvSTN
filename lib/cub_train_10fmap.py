...
#from model.model_CUB_10fmap import Net
#from model.temp import Net
...
def train_model(model, imdb, log, optimizer, epoch, batch_size):
    ...
    for batch_num in range(num_batches_per_epoch):
        ...
        optimizer.zero_grad()
        likelihood, x, all_boxes = model(data, epoch, "train", log, batch_id=count+1, batch_size=data.shape[0])
...
def test_model(model, imdb, log, optimizer, epoch, batch_size, area_hist, indicator=None, rgb_image=None):
    ...
    with torch.no_grad():
        ...
        for batch_num in range(num_batches_per_epoch):
            ...
            likelihood, x, all_boxes = model(data, epoch, "test", log, batch_id=count+1, batch_size=data.shape[0])
            ...
            #        x[offset], rgb_image, fmap, 320, epoch)

            #save the selected boxes as detection for each image, for each class
            #for i in range(len(selected_boxes)):
            #    detection = box_utils.get_class_specific_boxes(selected_boxes[i], all_boxes, detection, x, i+1, fmap, start_index)
            #detection = box_utils.get_detected_boxes_cub(all_boxes, x, detection, likelihood, start_index, data.shape[0])
        ...
        if indicator is None:
            ...
            if loca_acc > best_loc_acc1:
                ...
                torch.save(model.state_dict(), "/export/livia/Database/CUB_200_2011/model_ckpt_10fmap.pth")
            indicator = np.arange(test_size)
            vis_utils_cub.plot_bboxes_of_few_images_cub(imdb, detection_max, selected_indices, imagepath, epoch, "corloc", "corLoc", indicator)
            ...
            if loca_acc > best_loc_acc2:
                ...
                #vis_utils_cub.plot_bboxes_of_few_images_cub(imdb, detection_max, model.box_pos, indices, imagepath, epoch, "corloc", "corLoc", indicator)
            vis_utils_cub.plot_bboxes_of_few_images_cub(imdb, detection_max, selected_indices2, imagepath, epoch, "corloc", "corLoc", indicator)
            ...
            #indices = np.arange(test_size)
            #vis_utils_cub.plot_bboxes_of_few_images_cub(imdb, detection, indices, imagepath, epoch, "corloc", "corLoc", indicator)
...
if __name__=="__main__":
    ...
    fh = logging.FileHandler('running_10fmap.log', mode='w')
