...
class Net(nn.Module):
    def __init__(self):
        ...
        #self.fc1 = nn.Linear(fmap*fmap*256, 201)
        ...
        #self.conv_stn = nn.Conv2d(2048, 256, kernel_size=3)
        #self.conv_stn2 = nn.Conv2d(2048, 2048, kernel_size=3, stride=3)
        #self.conv_stn = nn.Conv2d(2048, 201, kernel_size=3/2)
        ...
        self.fc_GAP = nn.Linear(2048*fmap*fmap, self.n_classes+1)
    ...
    def stn(self, x, epoch, flag, batch_id, log):
        ...
        no_rotation = torch.zeros(theta[:, 1].shape).cuda()
        ...
        list_of_tensors = [theta[:, 0], no_rotation, theta[:, 1], no_rotation,
                            theta[:, 2], theta[:, 3]]
    ...
    def forward(self, x, epoch, flag, log, batch_id=0, batch_size=4):
        ...
        x = self.GAP(x)
        x_flat = x.contiguous().view(-1, fmap*fmap*2048)
        x_flat = self.fc_GAP(x_flat)
        classif_scores = F.softmax(x_flat, dim=1)
        #with conv reduction
        ...
        """
        with torch.no_grad():
           all_boxes = box_utils_cub.get_transformed_bounding_box_cub(self.box_pos, theta, classif_scores, batch_size, self.grid_positions_is[0].detach(), H, W, epoch, fmap, x)
        #all_boxes = torch.zeros(batch_size, 5)
        return classif_scores, x, all_boxes, theta
        #return classif_scores, 1, 1, 1
