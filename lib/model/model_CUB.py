...
class Net(nn.Module):
    ...
    def forward(self, x, epoch, flag, log, batch_id=0, batch_size=4):
        ...
        """
        with torch.no_grad():
            all_boxes = box_utils_cub.get_transformed_bounding_box_cub(self.box_pos, theta, classif_scores, batch_size, 
self.grid_positions_is[0].detach(), H, W, epoch, fmap, x)
